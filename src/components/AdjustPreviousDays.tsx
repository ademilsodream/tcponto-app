...
import { ArrowLeft, Calendar as CalendarIcon, AlertTriangle, Clock, Save, Edit3, MapPin } from 'lucide-react'; // Adicionado MapPin
import { format, startOfMonth, endOfMonth, subDays, isAfter, isBefore } from 'date-fns';
import { ptBR } from 'date-fns/locale';
import { useAuth } from '@/contexts/AuthContext';
import { supabase } from '@/integrations/supabase/client';
import { useToast } from '@/hooks/use-toast';
// Importar componentes Select do shadcn/ui
import {
  Select,
  SelectContent,
  SelectItem,
...
  // Não precisamos da localização aqui, pois a edição é uma solicitação
...
  clock_out: string;
  reason: string;
  locationName: string; // Adicionado para a localização da solicitação
}


// Interface para as localizações permitidas
interface AllowedLocation {
  id: string;
  name: string;
  address: string; // Manter para referência, mas não usado na edição
  latitude: number; // Manter para referência, mas não usado na edição
  longitude: number; // Manter para referência, mas não usado na edição
  range_meters: number; // Manter para referência, mas não usado na edição
  is_active: boolean;
}
...
  // Nova função para carregar localizações permitidas
...
      
      // Verificar autenticação (redundante se user do context estiver ok, mas boa prática)
...
      // Mapeamento dos campos para os valores corretos do banco (nomes usados na coluna 'field')
      const fieldMapping = {
        clock_in: 'clockIn',
...
        status: 'pending',
        location_name: editForm.locationName, // Adiciona a localização à solicitação
      };
