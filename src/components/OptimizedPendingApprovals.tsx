...
import { Json } from '@/types/supabase'; // Assuming you have a Json type defined
...
// Interface for the expected structure in the 'location' column of 'edit_requests'
...
interface RawEditRequestData {
  ...
  location: Json | null; // Use Json or a more specific DB type if available
}
...
interface EditRequest {
  ...
  status: 'pending' | 'approved' | 'rejected';
  location?: EditRequestLocation | null; // Mapped from location (structure might need refinement)
}
...
const mapFieldDbToCamelCase = (dbField: string): 'clockIn' | 'lunchStart' | 'lunchEnd' | 'clockOut' => {
  switch (dbField) {
    ...
    default:
      ...
      // Depending on requirements, you might filter these out or map to a default
      // Casting as a fallback is risky but might be necessary if DB data isn't strictly controlled
...
const OptimizedPendingApprovals: React.FC<PendingApprovalsProps> = ({ employees, onApprovalChange }) => {
  ...
  const {
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('edit_requests')
        .select('*') // Fetch all columns including location
      ...
      return (data as RawEditRequestData[]).map(request => ({
        ...
        location: request.location as EditRequestLocation | null, // Cast location structure
      }));
    },
  ...
  const { pendingRequests, processedRequests, groupedPendingRequests } = useMemo(() => {
    ...
    // The 'forEach' error should be resolved here.
  ...
  const handleGroupApproval = useCallback(async (group: GroupedRequest, approved: boolean) => {
    try {
      ...
      if (approved) {
        // Fetch existing time record efficiently, including current location
        const { data: timeRecord, error: fetchError } = await supabase
          .from('time_records')
          .select('id, location') // Include location
        ...
        let mergedLocations: any = timeRecord?.location || {}; // Start with existing location or empty object
        ...
        for (const request of group.requests) {
          ...
          // If the edit request has location data for this field, merge it
        ...
        updateData.location = Object.keys(mergedLocations).length > 0 ? mergedLocations : null;
        ...
        if (timeRecord) {
          ...
          const { error: updateRecordError } = await supabase
            .from('time_records')
            .update(updateData) // updateData includes merged location
            .eq('id', timeRecord.id);
          ...
          const { error: insertError } = await supabase
            ...
            .insert({
              ...
              date: group.date,
              ...updateData, // updateData includes merged location
              // 'location' is already in updateData
            });
