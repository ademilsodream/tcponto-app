...
  // CORRIGIDO: Ajustada a estrutura try...catch...finally e callbacks
...
        setSubmitting(false); // Garante que submitting seja false no final
      }
    },
    500 // Delay do debounce
  );
...
      return; // Evita múltiplos cliques enquanto enviando
    }
...
        variant: "default" // Ou 'warning' se tiver
      });
      return;
    }
...
    setSubmitting(true);
    console.log(`Attempting to register ${action} at ${localTime} on ${localDate}`);




    // Passa os callbacks de sucesso e erro para o debouncedLocationRequest
    debouncedLocationRequest(action,
      // callback onSuccess - trata sucesso na validação de localização
...
        const { location, closestLocation, distance, gpsAccuracy, adaptiveRange } = locationValidationResult;
...
          distance: distance || -1, // Salva a distância se disponível
          latitude: location.latitude,
          longitude: location.longitude,
          timestamp: new Date().toISOString(), // Timestamp ISO 8601
          locationName: closestLocation?.name || 'Localização Desconhecida', // Salva o nome da localização permitida mais próxima
          // gps_accuracy: gpsAccuracy // Opcional, se quiser salvar
        };
...
        try {
          // Lógica para atualizar ou inserir no Supabase
          if (timeRecord) {
            // Atualiza o registro existente
            const newTimeRecord = {
              ...timeRecord,
              [action]: localTime,
              // Atualiza a coluna locations mesclando os dados existentes com o novo ponto
              locations: {
                ...(timeRecord.locations as LocationsData | null), // Cast para LocationsData ou null
                [action]: locationData
              },
              updated_at: new Date().toISOString() // Atualiza timestamp de atualização
            };
...
            setTimeRecord(data); // Atualiza o estado local com o registro retornado pelo Supabase
...
          } else {
            // Insere um novo registro (apenas para clock_in)
            if (action !== 'clock_in') {
              console.warn(`Ação ${action} tentada sem registro de clock_in.`);
              toast({
                title: "Erro",
...
            const newRecord: Partial<TimeRecord> = { // Usar Partial pois nem todos os campos são obrigatórios na criação
              user_id: user.id,
              date: localDate,
              clock_in: localTime,
              total_hours: 0, // Inicializa com 0
              locations: { [action]: locationData } as Json, // Salva a localização inicial como Json
              status: 'open', // Status inicial
              is_pending_approval: false // Não pendente na criação
            };
...
          // Refetch para garantir que os dados locais estejam sincronizados
          refetchRecord();
